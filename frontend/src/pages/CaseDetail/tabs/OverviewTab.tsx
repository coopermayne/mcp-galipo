...
export function OverviewTab({ caseData, caseId, constants, onUpdateField }: OverviewTabProps) {
  ...
  // Delete targets
  const [deleteTarget, setDeleteTarget] = useState<{ id: number; name: string; type: string; role?: string } | null>(null);
  ...
  const handleRemovePerson = useCallback((person: CasePerson, type: string) => {
    setDeleteTarget({ id: person.id, name: person.name, type, role: person.role });
  }, []);

  const confirmDelete = useCallback(() => {
    if (deleteTarget) {
      removePersonMutation.mutate({ personId: deleteTarget.id, role: deleteTarget.role });
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to remove "${deleteTarget?.name}" from this case?`}
        confirmText="Remove"
        variant="danger"
        isLoading={removePersonMutation.isPending}
      />
    </div>
  );
}
