...
export function QuickCaseSearch({ isOpen, onClose }: QuickCaseSearchProps) {
  ...
  const getIcon = () => {
    ...
    return <Search className="w-5 h-5 text-text-muted" />;
  };
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh]">
      ...
      <div className="relative w-full max-w-lg bg-bg-surface rounded-lg shadow-2xl overflow-hidden">
        ...
        {mode !== 'search' && selectedCase && (
          <div className="px-4 py-2 bg-bg-hover border-b border-border flex items-center gap-2">
        ...
        <div className="flex items-center px-4 border-b border-border">
          ...
          <input
            ...
            className="flex-1 px-3 py-4 bg-transparent text-text placeholder-text-muted focus:outline-none disabled:opacity-50"
          />
          {mode === 'search' && (
            <button
              onClick={onClose}
        ...
        {/* Footer with hints */}
        <div className="px-4 py-2 border-t border-border text-xs text-text-muted flex flex-wrap gap-x-4 gap-y-1">
          {mode === 'search' ? (
            <>
              ...
              {filteredCases.length > 0 && (
                <>
                  <span><kbd className="px-1.5 py-0.5 bg-bg-hover rounded">{modKey}T</kbd> task</span>
                  <span><kbd className="px-1.5 py-0.5 bg-bg-hover rounded">{modKey}E</kbd> event</span>
                </>
              )}
            ...
            <>
              <span><kbd className="px-1.5 py-0.5 bg-bg-hover rounded">↵</kbd> create</span>
              <span><kbd className="px-1.5 py-0.5 bg-bg-hover rounded">esc</kbd> back</span>
            </>
          )}
        </div>
      </div>
