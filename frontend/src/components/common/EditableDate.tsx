  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={`
          inline-flex items-center gap-1.5 px-2 py-1 rounded text-sm text-text
          ${!hasDate ? 'text-text-muted italic' : ''}
          ${disabled ? 'opacity-60' : ''}
        `}
      >
        <button
          ...
          className={`
            ...
            ${disabled ? 'cursor-default' : 'cursor-pointer hover:text-text-secondary'}
          `}
        >
...
export function EditableDate({
  ...
  if (disabled) {
    return (
      <div className={`inline-flex items-center gap-1.5 px-2 py-1 text-sm text-text opacity-60 ${className}`}>
        <Calendar className="w-3.5 h-3.5 shrink-0" />
