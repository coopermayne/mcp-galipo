  ...
  return (
    <div
      ...
      className={`
        ...
        ${isDragging ? 'shadow-lg rounded-lg bg-bg-surface border border-primary-500' : ''}
        ${isHighlighted ? 'bg-primary-50 dark:bg-primary-900/20' : ''}
        ${showDragHandle && isTouchDevice ? 'touch-none active:bg-bg-hover active:scale-[0.98]' : ''}
    ...
    >
        ...
        <div className={`hidden md:flex w-6 flex-shrink-0 items-center justify-center pt-0.5 transition-opacity ${isHovered && showDragHandle ? 'opacity-100' : 'opacity-0'}`}>
          {showDragHandle && (
            <button
              {...attributes}
              {...listeners}
              className={`p-0.5 text-text-muted ${disableDrag ? 'cursor-default' : 'cursor-grab active:cursor-grabbing hover:text-text-secondary'}`}
      ...
      <div className="flex-1 min-w-0 pt-0.5">
        ...
        <div className={`text-sm leading-snug truncate transition-all duration-200 ${showCompleted ? 'text-text-muted line-through' : 'text-text'}`}>
          {task.description}
        </div>
          ...
          {task.assignee && (
            ...
            {isDone ? (
              ...
              isTouchDevice ? (
                ...
                dateInfo ? (
                  <span className={`flex items-center gap-1 text-xs ${dateInfo.isOverdue ? 'text-red-500' : 'text-text-muted'}`}>
                    <Calendar className="w-3 h-3 flex-shrink-0" />
            ...
            {!isDone && (task.has_events || task.event_id) && (
              isTouchDevice ? (
                ...
                <span className={`flex items-center gap-1 text-xs ${
                  task.event_id ? 'text-primary-500' : 'text-text-muted'
                }`}>
                ...
                <div className="relative group/event">
                  ...
                  {task.event_id && task.event_description && (
                    <div className="absolute left-0 bottom-full mb-1 px-2 py-1 text-xs bg-bg-elevated text-white rounded shadow-lg whitespace-nowrap z-50 opacity-0 group-hover/event:opacity-100 pointer-events-none transition-opacity">
            ...
            {!isDone && (
              isTouchDevice ? (
                // Mobile: static display
                <span className={`flex items-center gap-1 text-xs ${
                  PRIORITY_OPTIONS.find(p => p.value === task.urgency)?.color || 'text-text-muted'
                }`}>
