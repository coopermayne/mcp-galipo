...
export interface TaskItemProps {
  ...
  /** Callback when checkbox is clicked on a done task (mark pending) */
...
export function TaskItem({
  ...
  const handleCheckboxClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (isDone && onMarkPending) {
      onMarkPending(task.id);
    } else if (!isDone && onMarkDone) {
      onMarkDone(task.id);
    }
  };
  ...
  return (
    ...
    >
      ...
      <button
        ...
        `}
        title={isDone ? 'Mark as pending' : (showCompleted ? 'Completing...' : 'Mark as done')}
      >
        {showCompleted && (
          <svg
