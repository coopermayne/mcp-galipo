...
class SearchInput(BaseModel):
    ...
    category: Optional[str] = Field(None, description="(persons) Filter by role category: client, internal_team, opposing_team, third_party")
...
def register_tools(mcp):
    ...
    def manage_case_role(context: Context, data: ManageCaseRoleInput) -> dict:
        ...
        Role is specified by NAME (e.g. "Client", "Defense Counsel") — resolved
        to role_id internally. NOT for judges — use manage_proceeding instead.

        Examples:
        - manage_case_role(action="assign", case_id=1, person_id=5, role="Client", is_primary=true)
        - manage_case_role(action="update", case_id=1, person_id=5, role="Client", case_notes="Prefers email")
        - manage_case_role(action="change_role", case_id=1, person_id=5, role="Witness", new_role="Plaintiff Expert")
        - manage_case_role(action="remove", case_id=1, person_id=5, role="Client")
        """
